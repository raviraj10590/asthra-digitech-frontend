...
• Objections: costly/ದುಬಾರಿ → quality+support value, ಅವರ budget ಕೇಳಿ; "think ಮಾಡ್ತೀನಿ" → ಒಂದು ಸೂಕ್ತ ಕೆಲಸದ ಉದಾಹರಣೆ + free consultation offer; "ಬೇರೆ agency ಇದೆ" → ಗೌರವದಿಂದ differentiator (Kannada-first, politicians/govt ಅನುಭವ, in-house AI) + door open; "time ಇಲ್ಲ" → "ಎಲ್ಲಾ ನಾವೇ ನೋಡಿಕೊಳ್ಳುತ್ತೇವೆ".
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Extract lead info from this WhatsApp sales conversation. "
                        "Return ONLY a valid JSON object with these optional fields: "
                        "name, company, service_needed, budget, city, timeline, requirements, score, summary. "
                        "score: \"hot\" (budget or timeline given, urgency expressed, or meeting agreed), "
                        "\"warm\" (clear service need but budget/timeline unknown), "
                        "\"cold\" (vague browsing, no concrete need). "
                        "summary: ONE short English sentence a salesperson can read at a glance "
                        "(who they are, what they want, where the deal stands). "
...
SCORE_BADGES = {"hot": "🔥 HOT", "warm": "🌤 WARM", "cold": "❄️ COLD"}
...
def maybe_alert_lead(sender: str, lead: dict, already_alerted: bool):
    ...
    Now includes the sales score + one-line summary, and mirrors the summary
    into the CRM chat thread as a clearly-marked internal note."""
    ...
    score = str(lead.get("score", "")).lower()
    badge = SCORE_BADGES.get(score, "🔔 LEAD")
    lines = [f"{badge} — new lead captured by bot!", "", f"From: wa.me/{sender}"]
    ...
    if lead.get("summary"):
        log_reply_to_crm(sender,
            f"🤖 Internal lead note (not sent to customer) — {badge}: {lead['summary']}")
