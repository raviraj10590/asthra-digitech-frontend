...
from bic import brain, policy, db  # noqa: E402
...
class TestBrainRouting(unittest.TestCase):
    ...
    def test_unknown_sender_takes_client_flow(self):
        rec = []
        policy.invalidate(STRANGER)
        with mock.patch.object(db, "select", return_value=[]):
    ...
    def test_staff_takes_owner_flow(self):
        rec = []
        policy.invalidate("91777000")
        with mock.patch.object(db, "select", return_value=[{"role": "STAFF", "label": "s"}]):
